...
int main(int argc, char *argv[])
{
    ...
    QString appName("DetaperProject.exe");
    QProcess process;
    process.start("tasklist", QStringList() << "-fi"
                                            << "imagename eq " + appName);
    ...
    if (tasksList.contains(appName))
    {
        if (tasksList.count(appName) > 1)
        {
            ...
            QMessageBox mb(QMessageBox::Critical, "提示", "软件已经运行!", QMessageBox::Ok);
            mb.exec();
            return 0;
        }
    }
    ...
    TaskEngineeManager tem(nullptr, 8);
    ...
    VisionConfigManager visionConfigManager(AdaptVisionLocationConfig::staticMetaObject, dutRelatedVisionConfigDir);
    DetaperVisionFactory visionFactory(&visionConfigManager);
    ...
    DetaperMotionElementFactory motionElementFactory;
    ...
    engine.rootContext()->setContextProperty("gtCoreConfigs", GTCardCfgManager::getIns().getGtCardCfg()->gtCoreCfgs());
    ...
    DetaperDispatcher dispatcher(&masterMotionManager);
