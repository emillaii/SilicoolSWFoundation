...
#define CHECK_GT_PRT(ptr)                                                                                              \
    ...
    {
        throw SilicolAbort(
            tr("Only 1-8 axis can do interpolation. X axis index: %1").arg(m_xAxis->getGtAxisConfig()->index()));
    }
    if (m_yAxis->getGtAxisConfig()->index() > 8)
    {
        throw SilicolAbort(
            tr("Only 1-8 axis can do interpolation. Y axis index: %1").arg(m_yAxis->getGtAxisConfig()->index()));
    }
...
{
    throw SilicolAbort("Unimplemented function: flyDispense");
    ...
    //    mode.errorBand = 0;
    //    mode.outputPulseWidth = flyDispensingConfig->shotGlueDotTime() * 1000000;
    //    checkResult1(GTN_SetPosCompareMode(m_core, PSOIndex, &mode));

    //    int count = allLineLen / flyDispensingConfig->shotGlueDotStep();
    //    double step = allLineLen / count;
    ...
    //    comparePsoPrm.syncPos = step * m_xAxis->getGtAxisConfig()->scale();
...
void GTDispenser::appendLinePoint(const PathEndPoint &point)
{
    ...
    checkResult1(GTN_LnXY(m_core, m_crd, point.x * xAxisScale, point.y * yAxisScale,
                          point.maxVel * xAxisScale / VelCoeff, qMin(xMaxAcc, yMaxAcc),
