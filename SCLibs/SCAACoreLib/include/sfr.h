...
public:
    ...
    static vector<int> classifyLayers(std::vector<MTF_Pattern_Position> & vec, int x = 0, int y = 0, double threshold = 0.05);
    static bool sfr_calculation_single_pattern(cv::Mat cvimg, double &t_sfr, double &r_sfr, double &b_sfr, double &l_sfr, int samplePosition = 1, bool usinglpmmDomain = false, double lpmmFullRange = 1000, double targetlpmm=125);
private:
    sfr() {}
};
